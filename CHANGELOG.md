...
- **Filter, destination-group chips & navigator**: A **filter** box narrows the timeline to matching lanes/events. A **chip bar** offers a `◎ SHOW ALL` reset plus one chip per destination group — each **inheriting the room's declared schema colour** (from the DOM twist's `--lcars-color`) — to scope the view with one click. A **navigation minimap** (bottom-right) shows the whole span with event marks, the NOW tick, and a draggable viewport box for fast scrolling. Rendered **on demand** — a snapshot from the in-memory log + schedule each open (and on ⟳ Refresh), never on a live subscription. Design rationale: `docs/Audits/TimeLine Investigation.md`.
