...
- **Schedule on the timeline (the future)**: The production `SCHEDULE` is drawn as dashed **"planned" bands** in the future region — booked rooms on the WHERE side, booked crew on the WHO side — reachable by scrolling right or the `⇥ SCHEDULE` jump button; `⤒ NOW` re-centres on the playhead. Rendered **on demand** — a snapshot from the in-memory log + schedule each open (and on ⟳ Refresh), never on a live subscription. Design rationale: `docs/Audits/TimeLine Investigation.md`.
