...
export async function initSickBay(): Promise<void> {
  const groupColor = '255,51,51'; // RED ALERT color
  const hexColor = '#ff3333';
  const group = Footer.addGroup('SICK BAY', { color: groupColor, collapsed: true });
  if (!group) return;

  // Make the label pulse red
  group.labelEl.style.animation = 'sickbay-pulse 2s infinite alternate';
  if (!document.getElementById('sickbay-pulse-styles')) {
    const style = document.createElement('style');
    style.id = 'sickbay-pulse-styles';
    style.textContent = `
      @keyframes sickbay-pulse {
        0% { box-shadow: 0 0 5px rgba(255,51,51,0.4); background-color: rgb(255,51,51); color: #000; }
        100% { box-shadow: 0 0 20px rgba(255,51,51,1); background-color: rgb(255,0,0); color: #fff; }
      }
    `;
  ...
  Footer.addTab({ id: 'sick-bay-monitor', name: 'MONITOR' }, {
    ...
    onActivate: async () => {
      ...
      if (!pane) return;
      pane.innerHTML = `<div style="padding: 24px; color: ${hexColor};">Scanning all systems...</div>`;
      ...
      if (faults.length === 0) {
        pane.innerHTML = `
          <div style="padding: 24px; color: #888;">
            <h2 style="color: ${hexColor}; margin-top: 0; display: flex; align-items: center; gap: 8px;">
        ...
        <div style="padding: 24px;">
          ...
          <div style="display: flex; flex-wrap: wrap; gap: 16px; margin-top: 16px;">
            ${faults.map(f => {
              ...
              return `
                <div style="flex: 1 1 300px; max-width: 400px; background: rgba(255,0,0,0.1); border: 2px solid ${hexColor}; border-radius: 8px; padding: 12px; display: flex; flex-direction: column; gap: 8px; box-shadow: 0 0 10px rgba(255,51,51,0.2); cursor: pointer;" onclick="${clickScript}">
                  <div style="font-weight: bold; font-size: 1.2em; color: ${hexColor}; display: flex; justify-content: space-between; align-items: flex-start;">
                  ...
                  <div style="background: #000; border: 2px solid #555; border-radius: 6px; padding: 12px; text-align: center; margin-top: 8px;">
                    <div style="color: ${hexColor}; font-family: monospace; font-size: 1.2em; font-weight: bold; letter-spacing: 2px;">
                      ${f.status}
                    </div>
