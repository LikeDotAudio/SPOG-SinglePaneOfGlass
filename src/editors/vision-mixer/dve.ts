...
export function buildDveEditor(opts: DveEditorOpts): HTMLElement {
  ...
  const sliders = AXES.map((ax) => {
    ...
    range.addEventListener('input', () => {
      (work[editing] as DVEKeyframe)[ax.key] = +range.value;
  ...
  const play = el('button', { class: 'vm-tbtn take', type: 'button' }, ['▶ PLAY A→B']);
  ...
  function paint(): void {
    ...
    for (const s of sliders) {
      const v = Math.round((work[editing] as DVEKeyframe)[s.ax.key]);
