// src/editors/wysiwyg/render — the 60fps top-down pre-viz render loop.
//
// Extracted verbatim from view.ts: floor grid, foot-candle heat map, beam cones,
// camera frustum, virtual talent + ray-traced shadow, and per-fixture tally glow.
// The loop reads the live rig (fx) + overlay state (ui) and paints one frame.

import type { Fixture, UiState } from './state.js';
...
/**
 * Paint one pre-viz frame into `g` (sized from `cv`), reading the overlay flags in
 * `ui` and the live rig `fx`, and updating the `tag` readout. Called each rAF tick.
 */
